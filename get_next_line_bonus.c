...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/17 01:17:38 by mamazian          #+#    #+#             */
/*   Updated: 2024/11/18 04:09:18 by mamazian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_tracker	*tracker[FD_MAX];
	char				*line;

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, NULL, 0) < 0)
	{
		if (tracker[fd] && tracker[fd]->buffer[0])
		{
			tracker[fd]->buffer[0] = 0;
			free_wagons(&tracker[fd], NULL);
		}
		return (NULL);
	}
	train_of_buffer(&tracker[fd], fd);
	if (!tracker[fd])
		return (NULL);
	line = extract_line(tracker[fd]);
	train_clean(&tracker[fd]);
	return (line);
}
